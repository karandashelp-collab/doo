                ...
                if(AppConfig.all_live_tv_type == 0) {
                    if(mData.get(position).getType()== 1) {
                        if (mData.get(position).isPlay_Premium()) {
                            ...
                            HelperUtils helperUtils = new HelperUtils((Activity) mContext);
                            helperUtils.Buy_Premium_Dialog((Activity) mContext, "Buy Premium!", "Buy Premium Subscription To Watch Premium Content", R.raw.rocket_telescope);
                        }
                    } else {
                        if(mData.get(position).getStream_type().equals("Embed")){
                            Intent intent = new Intent(mContext, EmbedPlayer.class);
